...
export default function RecapBulananPage({
  ...
  onBack: () => void;
  onTabChange: (tab: "harian" | "tahunan") => void;
  onDashboard?: () => void;
  onAddExpense?: () => void;
}) {
    ...
    if (subPage === "awal") {
      return (
        ...
        >
          ...
          {/* Tab: Harian */}
          <button
            style={{ position: "absolute", zIndex: 20, left: 37, top: 113, width: 73, height: 21, borderRadius: 100, background: "transparent", border: "none", cursor: "pointer" }}
    ...
    if (subPage === "detail") {
      return (
        ...
        >
          ...
          {/* Fixed height of 1250px so size-full works correctly for the monthly detail layout */}
          <div className="rb-scroll" style={{ position: "relative", width: W, height: 1250 }}>
            ...
            {/* Navbar: Dashboard */}
            <button
              style={{ position: "absolute", zIndex: 20, left: 22, top: 1181, width: 150, height: 49, background: "transparent", border: "none", cursor: "pointer" }}
              onClick={goToDashboard}
            />
            {/* Navbar: FAB (Plus) */}
            <button
              style={{ position: "absolute", zIndex: 20, left: 170, top: 1171, width: 54, height: 54, borderRadius: "50%", background: "transparent", border: "none", cursor: "pointer" }}
              onClick={onAddExpense}
            />
            {/* Navbar: Recap */}
            <button
              style={{ position: "absolute", zIndex: 20, left: 220, top: 1181, width: 150, height: 49, background: "transparent", border: "none", cursor: "pointer" }}
              onClick={() => setSubPage("awal")}
            />
          </div>
        </div>
