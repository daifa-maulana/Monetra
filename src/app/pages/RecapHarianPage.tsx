...
export default function RecapHarianPage({
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6" style={{ background: "#f0ecff" }}>
      <div>
        {/* Phone frame — overflow:hidden keeps everything inside */}
        <div style={{ position: "relative", width: W, height: H, overflow: "hidden", borderRadius: 30, boxShadow: "0 25px 50px -12px rgba(0,0,0,0.25)", background: "#fdfdff" }}>
          ...
          {/* ── Scrollable content area (leaves 80px for navbar) ── */}
          <div style={{ position: "absolute", inset: 0, bottom: 80, overflowY: subPage === "detail" ? "auto" : "hidden", scrollbarWidth: "none" }}>
            ...
            {subPage === "detail" && (
              <div className="recap-scroll" style={{ position: "relative", width: W, height: 1040 }}>
                <RecapHarian3 />
                <button style={{ position: "absolute", zIndex: 20, left: 160, top: 113, width: 73, height: 21, background: "transparent", border: "none", cursor: "pointer" }} onClick={() => onTabChange("bulanan")} />
                <button style={{ position: "absolute", zIndex: 20, left: 283, top: 113, width: 73, height: 21, background: "transparent", border: "none", cursor: "pointer" }} onClick={() => onTabChange("tahunan")} />
              </div>
          ...
          {/* ── Back button (inside frame, on top of content) ── */}
          {subPage !== "awal" && (
            <button
              onClick={handleBack}
          ...
          {/* ── Bottom Navbar (always on top, covers Figma's static navbar) ── */}
