    ...
    if (subPage === "kalender") {
      return (
        <div style={{ position: "relative", width: W, height: H, overflow: "hidden" }}>
          ...
          />
          {/* Navbar FAB overlay over the center button inside RecapTahunan2 bottom nav */}
          <button
            style={{ position: "absolute", zIndex: 20, left: 170, top: 778, width: 54, height: 54, borderRadius: "50%", background: "transparent", border: "none", cursor: "pointer" }}
            onClick={onAddExpense}
          />
    ...
    if (subPage === "detail") {
      return (
        ...
        >
          ...
          <div className="rt4-scroll" style={{ position: "relative", width: W }}>
            {/* Added spacer to prevent back button from overlapping header content */}
            <div style={{ height: 40 }} />
            <div style={{ position: "relative", width: W, height: 1340 }}>
              <RecapTahunan4 />
              {/* Tab: Harian */}
              <button
                style={{ position: "absolute", zIndex: 20, left: 37, top: 113, width: 73, height: 21, borderRadius: 100, background: "transparent", border: "none", cursor: "pointer" }}
              ...
              {/* Navbar: Dashboard */}
              <button
                style={{ position: "absolute", zIndex: 20, left: 22, top: 1271, width: 150, height: 49, background: "transparent", border: "none", cursor: "pointer" }}
                onClick={goToDashboard}
              />
              {/* Navbar: FAB (Plus) */}
              <button
                style={{ position: "absolute", zIndex: 20, left: 170, top: 1261, width: 54, height: 54, borderRadius: "50%", background: "transparent", border: "none", cursor: "pointer" }}
                onClick={onAddExpense}
              />
              {/* Navbar: Recap */}
              <button
                style={{ position: "absolute", zIndex: 20, left: 220, top: 1271, width: 150, height: 49, background: "transparent", border: "none", cursor: "pointer" }}
                onClick={() => setSubPage("awal")}
              />
            </div>
          </div>
